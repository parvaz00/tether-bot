...
ANALYZE_PROMPT = (
    "تو یک مترجم و ادمین حرفه‌ای کانال خبری تلگرام هستی که به فارسی و انگلیسی کاملاً مسلطی. "
    "متن خبر زیر (که به انگلیسیه) رو بخون و دو کار انجام بده:\n"
    "1. مشخص کن دقیقاً به کدوم یک یا چندتا از این تگ‌ها مربوطه:\n"
    + "\n".join(f"   - {t}" for t in TAGS)
    + "\n2. کل متن رو به فارسیِ روان و کمی عامیانه ترجمه کن، دقیقاً مثل لحن ادمین‌های کانال‌های خبری "
    "تلگرام (نه رسمی و خشک مثل خبرگزاری‌ها)، طوری که بشه عیناً همون متن رو تو یه پست کانال گذاشت.\n"
    "قوانین ترجمه:\n"
    "- ساختار جمله رو کاملاً فارسی و طبیعی بچین، نه ترجمه‌ی کلمه‌به‌کلمه با ترتیب انگلیسی.\n"
    "- اسم‌ها، نهادها و اصطلاحاتی که معادل فارسی رایج دارن رو فارسی بنویس (مثلاً Federal Reserve "
    "بشه «فدرال رزرو»). اگه یه اسم خاص، نماد، یا اصطلاح تخصصی معادل فارسی جا‌افتاده‌ای نداره، اشکالی "
    "نداره همون کلمه‌ی انگلیسی رو بیاری، ولی باید درست و طبیعی وسط جمله‌ی فارسی جا بگیره، نه این‌که "
    "به‌هم‌ریخته یا بی‌ربط وسط جمله پرت بشه.\n"
    "- چیزی از محتوای خبر رو کم یا خلاصه نکن.\n\n"
